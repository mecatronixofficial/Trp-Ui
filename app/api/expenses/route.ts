...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const record = await createExpense({ ...payload, branch });
    return NextResponse.json({ record, summary: await getExpenseSummary({ branch }) });
  } catch (error: any) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, summary: await getExpenseSummary({ branch }) });
  } catch (error: any) {
